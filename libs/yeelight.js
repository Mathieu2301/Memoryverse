...
class Bulb extends EventEmitter {
  ...
  _onData(data) {
    try {
      ...
      if (data.length > 1) return data.forEach((el) => this._onData(el));
      data = data[0];
  ...
  set(state, time = 300) {
    this.sendCmd({
      params: [(state ? 'on' : 'off'), 'smooth', time],
      method: 'set_power'
    });
  }

  brightness(value, time = 300) {
    this.sendCmd({
      params: [value, 'smooth', time],
      method: 'set_bright'
    });
  }
  ...
  white(temp = 3500, time = 300) {
    this.sendCmd({
      params: ['ct', temp, 100, 'smooth', time],
      method: 'set_scene'
    })
  }

  flow(flow = "0,1,7000,1,42751,100,7000,1,11534591,100") {
    this.sendCmd({
      params: ['cf', 0, 0, flow],
      method: 'set_scene'
    })
